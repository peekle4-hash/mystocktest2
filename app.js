...
let graphMode = 'REALIZED'; // REALIZED | TOTAL
...
function sumByScope(items, scope) {
  ...
  for (const p of items) {
    ...
    cost += p.cost;
    real += p.realizedCum;
    if (Number.isFinite(p.unreal)) unreal += p.unreal;
  }
...
function computeMonthlySeries(asOfIso){
  // Returns map ym -> {ISA:{invest, realized, unreal}, NORMAL:{...}, ALL:{...}}
  const cm = closeMap?.[asOfIso] || {};

  // FIFO lots per company+account
  const lots = new Map(); // key -> [{qty, price, ym}]
  const month = new Map();

  function ensure(ym){
    if(!month.has(ym)){
      month.set(ym, {
        ISA:{invest:0, realized:0, unreal:0},
        NORMAL:{invest:0, realized:0, unreal:0},
        ALL:{invest:0, realized:0, unreal:0},
      });
    }
    return month.get(ym);
  }
  function accKey(account){
    return account === 'ISA' ? 'ISA' : 'NORMAL';
  }
  function addTo(ym, acc, field, val){
    const m = ensure(ym);
    m[acc][field] += val;
    m.ALL[field] += val;
  }
  function lotKey(company, account){
    return `${company}__${account}`;
  }

  // sort by date order
  const sorted = [...rows].filter(r=>r && r.company && r.date).sort((a,b)=>String(a.date).localeCompare(String(b.date)));
  for(const r of sorted){
    const ym = ymKey(r.date);
    if(!ym) continue;
    const acc = accKey(r.account);
    const company = String(r.company||'').trim();
    const side = (r.side||'BUY').toUpperCase();
    const price = num(r.price);
    const qty = num(r.qty);
    if(!company || !Number.isFinite(price) || !Number.isFinite(qty) || qty<=0) continue;

    const k = lotKey(company, acc);
    if(!lots.has(k)) lots.set(k, []);
    const q = lots.get(k);

    if(side === 'BUY'){
      addTo(ym, acc, 'invest', price*qty);
      q.push({qty, price, ym});
    } else { // SELL
      let sellQty = qty;
      while(sellQty > 0 && q.length){
        const lot = q[0];
        const take = Math.min(lot.qty, sellQty);
        const profit = (price - lot.price) * take;
        // realized profit belongs to SELL month (common accounting view)
        addTo(ym, acc, 'realized', profit);
        lot.qty -= take;
        sellQty -= take;
        if(lot.qty <= 1e-12) q.shift();
      }
      // if sell more than holdings, ignore extra (not supported)
    }
  }

  // unrealized profit at asOf date: attribute to BUY month of remaining lots
  for(const [k, q] of lots.entries()){
    if(!q.length) continue;
    const [company, acc] = k.split('__');
    const close = Number(cm[company]);
    if(!Number.isFinite(close)) continue; // missing close -> keep '-' later
    for(const lot of q){
      const profit = (close - lot.price) * lot.qty;
      // unreal belongs to lot.ym (buy month) for "총수익률" mode display
      addTo(lot.ym, acc, 'unreal', profit);
    }
  }
...
function buildMonthlyTableAndCharts(ledger){
  const asOfIso = $("asOfDate").value || todayISO();
  const monthMap = computeMonthlySeries(asOfIso);
  ...
  // helpers for ROI and cumulative
  function roi(profit, invest){
    if(!Number.isFinite(invest) || invest<=0) return null;
  ...
  const series = {
    ISA:{labels:[], mret:[], crec:[]},
    NORMAL:{labels:[], mret:[], crec:[]},
    ALL:{labels:[], mret:[], crec:[]},
  };

  const cum = {ISA:1, NORMAL:1, ALL:1};

  for(const ym of months){
    const m = monthMap.get(ym);
    for(const acc of ["ISA","NORMAL","ALL"]){
      const invest = m[acc].invest;
      const realized = m[acc].realized;
      const unreal = m[acc].unreal;

      let profit, r;
      if(graphMode === "REALIZED"){
        profit = realized;
        r = roi(profit, invest);
      } else { // TOTAL
      ...
      // If TOTAL mode but close missing for any relevant holding, we still can show based on realized+unreal computed; unreal only counted when close exists.
      // ROI should be '-' if invest=0
      const rr = (r === null ? null : r);
      series[acc].labels.push(ym);
      series[acc].mret.push(rr);

      if(rr === null){
        series[acc].crec.push(null);
      } else {
        cum[acc] = cum[acc] * (1 + rr);
        series[acc].crec.push(cum[acc]-1);
      }
    }

    // table row (use ALL only to keep table compact)
    if(tbody){
      const investAll = m.ALL.invest;
      const profitAll = (graphMode==="REALIZED") ? m.ALL.realized : (m.ALL.realized + m.ALL.unreal);
      const rAll = roi(profitAll, investAll);
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${ym}</td><td>${fmtNum(investAll)}</td><td>${fmtNum(profitAll)}</td><td>${fmtPct(rAll)}</td>`;
      tbody.appendChild(tr);
    }
  }

  // Update chart note if exists
  const note = document.getElementById("monthlyModeNote");
  if(note){
    note.textContent = (graphMode==="REALIZED")
      ? "※ 실현손익(매도/체결 기준)으로만 월별 수익률을 계산해요."
      : "※ 총수익률 = 실현손익 + (기준일 종가 기준 평가손익). 종가 미입력 종목은 평가가 반영되지 않아요.";
  }

  // Draw charts
  renderMonthlyCharts(series);
}
...
function renderMonthlyCharts(series){
  const ctxBar = document.getElementById("barMonthly")?.getContext("2d");
  const ctxLine = document.getElementById("lineCumulative")?.getContext("2d");
  if(!ctxBar || !ctxLine || typeof Chart === "undefined") return;

  const labels = series.ALL.labels;

  const barData = {
    labels,
    datasets: [
      { label: "ISA 월 수익률(%)", data: series.ISA.mret.map(v=>v===null? null : v*100) },
      { label: "일반 월 수익률(%)", data: series.NORMAL.mret.map(v=>v===null? null : v*100) },
      { label: "전체 월 수익률(%)", data: series.ALL.mret.map(v=>v===null? null : v*100) },
    ]
  };
  ...
  const lineData = {
    labels,
    datasets: [
      { label: "ISA 누적(%)", data: series.ISA.crec.map(v=>v===null? null : v*100) },
      { label: "일반 누적(%)", data: series.NORMAL.crec.map(v=>v===null? null : v*100) },
      { label: "전체 누적(%)", data: series.ALL.crec.map(v=>v===null? null : v*100) },
    ]
  };
  ...
  _barChart = new Chart(ctxBar, { type: "bar", data: barData, options: commonOpts });

  if(_lineChart) _lineChart.destroy();
  _lineChart = new Chart(ctxLine, { type: "line", data: lineData, options: commonOpts });
}
...
function addWatchRow(){
  watchlist.push({ company:"", close:"", date: todayISO() });
...
function renderWatchlist(){
  ...
  // optional: suggest companies from portfolio
  const asOfIso = document.getElementById("asOfDate")?.value || todayISO();
  const ledger = computeLedger(asOfIso); // uses global rows + closeMap
  const companies = getCompaniesInPortfolio(ledger);
  ...
  watchlist.forEach((row, idx)=>{
    ...
    inpC.type = "text";
    inpC.placeholder = companySet.size ? "예: " + companies[0] : "예: 삼성전자";
    inpC.value = row.company || "";
    inpC.addEventListener("input", ()=>{
      watchlist[idx].company = inpC.value;
      saveWatchlist(watchlist);
    });
    ...
    inpP.type = "number";
    inpP.step = "any";
    inpP.placeholder = "예: 71500";
    inpP.value = row.close ?? "";
    inpP.addEventListener("input", ()=>{
      watchlist[idx].close = inpP.value;
      saveWatchlist(watchlist);
    });
    ...
    inpD.type = "date";
    inpD.value = row.date || todayISO();
    inpD.addEventListener("change", ()=>{
      watchlist[idx].date = inpD.value || todayISO();
      saveWatchlist(watchlist);
    });
    ...
    btn.textContent = "삭제";
    btn.addEventListener("click", ()=>deleteWatchRow(idx));
    tdX.appendChild(btn);
